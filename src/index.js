...
async function fetchOutlineResult( startResponse ) {
	if ( startResponse && startResponse.jobId ) {
		...
		while ( Date.now() < deadline ) {
			const polled = await apiFetch( {
				path: `/redaquest/v2/blog/outline/status?jobId=${ encodeURIComponent( startResponse.jobId ) }`,
			} );
			if ( polled && ( polled.status === 'failed' || polled.error_code ) ) {
				throw polled;
			}
			if ( isValidOutline( polled && polled.outline ) ) {
				return polled;
			}
			if ( polled && polled.status === 'done' && ! isValidOutline( polled.outline ) ) {
				throw new Error( __( 'Outline generation returned incomplete data.', 'redaquest-connector' ) );
			}
			await new Promise( ( resolve ) => setTimeout( resolve, OUTLINE_POLL_MS ) );
		}
		throw new Error( __( 'Outline generation timed out. Please try again.', 'redaquest-connector' ) );
	}

	if ( isValidOutline( startResponse && startResponse.outline ) ) {
		return startResponse;
	}

	if ( startResponse && startResponse.status === 'pending' ) {
		throw new Error( __( 'Outline is still processing. Update the RedaQuest Connector plugin and try again.', 'redaquest-connector' ) );
	}
...
function RedaQuestBlogModal() {
	...
	const onOutline = async () => {
		...
		try {
			const start = await apiFetch( {
				path: '/redaquest/v2/blog/outline',
				method: 'POST',
				data: { ...base, webResearch },
			} );
